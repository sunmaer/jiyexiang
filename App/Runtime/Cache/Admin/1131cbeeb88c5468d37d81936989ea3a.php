...
<body>
    ...
    <table  class="alluser-table">
        ...
        <tr>
            ...
            <th>数据库名称</th>
            <th  class="namehidden">备份时间</th>
            <th  class=" comphonehidden">修改时间</th>
            <th>文件大小</th>
            <th>操作</th>
        </tr>
        </thead>
        ...
        <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr class="gradeU">
            ...
            <td   class="namehidden"><?php echo (date( "Y-m-d H:i:s",$vo["ctime"])); ?></td>
            <td   class=" comphonehidden"><?php echo (date( "Y-m-d H:i:s",$vo["mtime"])); ?></td>
            <td><?php echo ($vo["size"]); ?> KB</td>
            <td>
            <a href="<?php echo U('System/doReduction',array('name'=>$vo['name']));?>" ><button  class="databutton" type="button">还原</button></a>
            <a href="<?php echo U('System/delData',array('name'=>$vo['name']));?>"><button class="databutton" type="button">删除</button></a>
            </td>
        </tr><?php endforeach; endif; ?>
        </tbody>
        </table>
<div class="backup">
    <form method="post"action="<?php echo U('System/doBackups');?>" enctype="multipart/form-data">
        <label>备注名称:</label><br>
        <input type="text" name="name" placeholder="数据库名称不要写中文！"><input type="submit" value="开始备份">
    </form>
</div>
</div>
			<div id="footer">
